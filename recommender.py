...
# ── 1. Integrations ──────────────────────────────────────────────────────────
try:
    from filter_engine import filter_eligible
except ImportError:
    try:
        from app.services.filter_engine import filter_eligible  # type: ignore[no-redef]
    except ImportError:
        raise ImportError("Could not import 'filter_eligible' from filter_engine.py")

try:
    from ranker import UserProfile, rank_schemes
except ImportError:
    try:
        from app.services.ranker import UserProfile, rank_schemes  # type: ignore[no-redef]
    except ImportError:
...
try:
    ...
    try:
        from app.services.fallback_engine import find_closest_schemes  # type: ignore[no-redef]
    except ImportError:
